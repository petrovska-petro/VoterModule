...
def test_set_gov(voter_module, governance, accounts):
    voter_module.setGovernance(accounts[3], {"from": governance})
    assert voter_module.governance() == accounts[3]


def test_set_gov_random_account(voter_module, accounts):
    with reverts("not-governance!"):
        voter_module.setGovernance(accounts[3], {"from": accounts[6]})
